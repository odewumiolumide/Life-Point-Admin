...
document.addEventListener("DOMContentLoaded", () => {
  ...
  parseBulkBtn?.addEventListener("click", () => {
    ...
    lines.forEach(line => {
      ...
      if (parts.length === 6) {
        ...
        const card = document.createElement("div");
        card.className = "mb-2 p-2 border rounded question-card";
        card.dataset.key = `q${Date.now()}${Math.floor(Math.random()*1000)}`;
        card.innerHTML = `
          <label>Question ${qNumber}</label>
          <textarea class="form-control mb-1 question-text">${qText}</textarea>
          <input class="form-control mb-1 option" value="${optA}" placeholder="Option A">
          <input class="form-control mb-1 option" value="${optB}" placeholder="Option B">
          <input class="form-control mb-1 option" value="${optC}" placeholder="Option C">
          <input class="form-control mb-1 option" value="${optD}" placeholder="Option D">
          <select class="form-select mb-1 correct-answer">
            <option value="">Correct Answer</option>
            <option ${correct==="A"?"selected":""}>A</option>
            <option ${correct==="B"?"selected":""}>B</option>
            <option ${correct==="C"?"selected":""}>C</option>
            <option ${correct==="D"?"selected":""}>D</option>
          </select>
          <button class="btn btn-sm btn-danger delete-question">Delete Question</button>
        `;
        questionsContainer.appendChild(card);
    ...
    bulkQuestions.value = "";
    showSuccess(`${count} questions added successfully!`);
  });
  ...
  document.getElementById("confirmDeleteQuestionBtn")?.addEventListener("click", async () => {
    ...
    if (testSelector.value) await remove(ref(db, `Exam/${testSelector.value}/questions/${qKey}`));
  ...
  async function setStatus(status) {
    ...
    statusBadge.className = "badge " + (status==="Published"?"bg-success":status==="Archived"?"bg-danger":"bg-warning");
    addQuestionBtn.disabled = status==="Published";
    questionsContainer.querySelectorAll("textarea,input,select").forEach(el => el.disabled = status==="Published");
